...
The logic module will contain all of the core functions. The words module will include the word lists that I will implement in the game. These modules will be imported into the game module, which will have the menu loop, gameplay loop, and takes in inputs. They will be stored in the src package. The only other module used will be the random module. 
